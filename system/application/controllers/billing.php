...
class Billing extends Controller
{
	...
	function firm_edit()
	{
		$sql = "SELECT firm.id,firm.dogovor,firm.address, firm.name,  firm.telefon, firm.rnn, firm.dogovor_date, firm.nomer1c, firm.bik FROM industry.firm WHERE  firm.id=".$this->uri->segment(3); 
		$this->db->where('id',$this->uri->segment(3));
	...
	function schetfactura()
	{
		if (isset($_POST['akt_vypolnenyh_rabot'])) {
					if (strlen(trim($_POST['akt_num'])) > 0) {
						$data['akt_num'] = $_POST['akt_num'];
					} else {
						$data['akt_num'] = -1;
					}
	...
	function schetoplata()
	{
		...
		$this->db->where('id',$data['firm']->bank_id);
		$data['bank']=$this->db->get("industry.bank")->row();
		$data['schet']=!isset($_POST['schet'])?"":"-ФАКТУРА";
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function migration()
    {
        ...
        $this->export_to_excel("other_reports/migration", $data,"енбекшильдерский_рэс");
